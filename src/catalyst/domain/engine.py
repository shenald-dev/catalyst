...
class WorkflowEngine:
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        for generation in nx.topological_generations(self.graph):
            # Run all tasks in the current generation concurrently
            generation_results = await asyncio.gather(
                *(self._run_task(node) for node in generation)
            )
            # Store results mapped to their node names
            for node, result in zip(generation, generation_results):
                results[node] = result
