...
class DAG:
    ...
    def validate(self) -> None:
        ...
        # Detect cycles using iterative DFS to avoid recursion depth issues
        ...
        for start in self._nodes:
            ...
            stack = [(start, iter(self._nodes[start].dependents))]
            ...
            while stack:
                ...
                try:
                    ...
                    if state.get(v, 0) == 0:
                        ...
                        state[v] = 1
                        parent[v] = u
                        stack.append((v, iter(self._nodes[v].dependents)))
                    elif state.get(v) == 1:
                        # Found a back edge to a node currently on stack (cycle)
                        cycle = self._reconstruct_cycle(parent, u, v)
                        raise CyclicDependencyError(
                            f"Cyclic dependency detected: {' -> '.join(cycle)}"
                        )
                    # If state == 2 (finished), ignore
                except StopIteration:
                    # Finished neighbors of u
                    stack.pop()
                    state[u] = 2  # visited
        return True
